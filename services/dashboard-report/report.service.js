...
const getSellsReport = async (fromDate, toDate) => {
  try {
    ...
    const sellStats = await InvoiceOverview.find({
      ...
      .select(
        "invoiceId clientId invoiceDate subTotalAmount taxAmount discountAmount grandTotalAmount"
      )
      .lean();
...
const getCollectionsReport = async (fromDate, toDate) => {
  try {
    ...
    // Get unique client IDs
    const clientIds = [
      ...new Set(collectionStats.map((stat) => stat.clientId)),
    ];
...
const getClientCollectionsReport = async (fromDate, toDate, clientId) => {
  try {
    ...
    throw new Error(
      `Error fetching client collections report: ${error.message}`
    );
  }
};
...
const getParticularsReport = async (fromDate, toDate, q) => {
  try {
    const match = {};
    const from = new Date(fromDate);
    const to = new Date(toDate);
    ...
    if (from && to) {
      match.createdDate = { $gte: from, $lt: nextDayUTC(to) };
    } else if (from) {
      match.createdDate = { $gte: from };
    } else if (to) {
      match.createdDate = { $lt: nextDayUTC(to) };
    }

    // Wildcard on perticulars (optional)
    if (q && q.trim()) {
      match.perticulars = { $regex: q.trim(), $options: "i" };
    }
    const pipeline = [
      { $match: match },
      {
        $group: {
          _id: '$perticulars',
          quantitySum: { $sum: { $ifNull: ['$quanity', 0] } },        // uses your field "quanity"
          discountTotalSum: { $sum: { $ifNull: ['$discountTotal', 0] } }
        }
      },
      {
        $project: {
...
const getClientOutstandingReport = async () => {
  try {
    ...
    const pipeline = [
      ...
      },
      {
        $project: {
          ...
          paymentAmount: 1,
          outstandingAmount: {
            $subtract: ["$purchasedAmount", "$paymentAmount"],
          },
        },
      },
      {
    ...
    const clientOutstandingAmounts = await ClientOutstanding.aggregate(
      pipeline
    );

    // Add row numbers (equivalent to @rownum in SQL)
    const clientOutstandingAmountsWithSlNo = clientOutstandingAmounts.map(
      (item, index) => ({
        slNo: index + 1,
    ...
    const purchasedAmount = clientOutstandingAmountsWithSlNo.reduce(
      (sum, item) => sum + item.purchasedAmount,
      0
    );
    const paymentAmount = clientOutstandingAmountsWithSlNo.reduce(
      (sum, item) => sum + item.paymentAmount,
      0
    );
    const outstandingAmount = clientOutstandingAmountsWithSlNo.reduce(
    ...
    throw new Error(
      `Error fetching client outstanding report: ${error.message}`
    );
  }
};
...
const getClientTradeBookReport = async (clientId, fromDate, toDate) => {
  try {
    ...
    const openingBalance =
      (invoiceSum[0]?.total || 0) - (paymentSum[0]?.total || 0);
    ...
    const [invoices, payments] = await Promise.all([
      InvoiceOverview.find({
        ...
        .select(
          "invoiceId clientId grandTotalAmount invoiceDate subTotalAmount discountAmount"
        )
        .lean(),
      Payment.find({
        clientId: clientId,
    ...
    throw new Error(
      `Error fetching client trade book report: ${error.message}`
    );
  }
};
